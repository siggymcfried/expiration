...
RSpec.describe ExpirationDatesController do
  ...
  describe 'GET show' do
    context 'when the food exists' do
      before { FactoryBot.create(:food, name: 'strawberries', created_at: 6.months.ago, expiration: 4.month.ago) }

      it 'calculates the date' do
        get :show, params: { id: 'strawberries' }, session: { user_id: user.id }, format: :json

        expect(response.body).to eq({ expires_on: 2.months.from_now.to_date }.to_json)
      end
    end
